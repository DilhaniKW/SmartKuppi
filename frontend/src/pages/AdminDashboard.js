...
const Sidebar = ({ isSidebarOpen, setIsSidebarOpen, admin, handleLogout, getInitials, navLinks, location }) => ( <
    ...
    p className = "px-2 mb-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest" > Main Menu < /p> {
        navLinks.map((link) => {
            ...
            return ( <
                ...
                /div> {
                    link.badge > 0 && ( <
                        span className = "px-2 py-0.5 text-[10px] font-bold bg-brand-500 text-white rounded-full" > { link.badge } <
                        /span>
                    )
                } <
                /Link>
    ...
    <
    div className = "w-10 h-10 rounded-full bg-brand-500 flex items-center justify-center text-white font-bold border border-brand-400" > { admin ? getInitials(admin.name) : 'A' } <
    /div> <
...
const Header = ({ isSidebarOpen, setIsSidebarOpen, admin, getInitials, handleLogout, title, showSearch = false }) => {
    ...
    return ( <
        ...
        <
        div className = "w-8 h-8 rounded-lg bg-brand-500 flex items-center justify-center text-white font-bold text-xs" > { admin ? getInitials(admin.name) : 'AD' } <
        /div> <
...
const TutorDetailModal = ({ tutor, onClose, onStatusChange, actionLoading, getInitials, getStatusBadge }) => {
    ...
    return ( <
        ...
        div className = "flex items-center space-x-4" >
        <
        div className = "w-16 h-16 rounded-2xl bg-brand-500 flex items-center justify-center text-white font-bold text-xl border-2 border-brand-400/50" > { getInitials(tutor.name) } <
        /div> <
        div >
        <
            ...
            tutor.subjects ? .length > 0 && ( <
                ...
                div className = "flex flex-wrap gap-2" > {
                    tutor.subjects.map((subject, i) => ( <
                        span key = { i }
                        className = "text-xs font-semibold bg-brand-50 text-brand-700 px-3 py-1 rounded-full border border-brand-100" > { subject } <
                        /span>
                    ))
                } <
                /div> <
...
const AdminDashboard = () => {
        ...
        const DashboardView = () => ( <
                ...
                animate = {
                    { opacity: 1, y: 0 } }
                className = "max-w-7xl mx-auto space-y-8" > { /* Page Header */ } <
                div className = "flex flex-col md:flex-row md:items-center justify-between gap-4" >
                <
                div >
                <
                h1 className = "text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight" > Dashboard Overview < /h1> <
                p className = "text-slate-500 mt-1" > Welcome back, { admin ? .name ? .split(' ')[0] || 'Admin' }.Here 's what'
                s happening today. < /p> <
                /div> <
                div className = "flex items-center space-x-3" >
                <
                ...
                <
                div className = "grid grid-cols-1 lg:grid-cols-3 gap-8" > { /* Pending Tutor Approvals Table */ } <
                ...
                div className = "overflow-x-auto" > {
                    pendingTutors.length > 0 ? ( <
                        ...
                        tbody className = "divide-y divide-slate-50" > {
                            pendingTutors.slice(0, 4).map((tutor) => ( <
                                    ...
                                    div className = "flex items-center space-x-3" >
                                    <
                                    div className = "w-10 h-10 rounded-xl bg-brand-100 flex items-center justify-center text-brand-600 font-bold text-xs border border-brand-200" > { getInitials(tutor.name) } <
                                    /div> <
                                    div >
                                    <
                                    ...
                                    div className = "flex flex-wrap gap-1" > {
                                        ...
                                        title = "Approve" > < CheckCircle className = "h-5 w-5" / > < /button> <
                                            button onClick = {
                                        ...
                                        title = "Details" > < MoreVertical className = "h-5 w-5" / > < /button> <
                                            /div> <
                                            /td> <
                                            /tr>
                                    ))
                    ...
                    { /* Sidebar Widgets */ } <
                    div className = "space-y-8" > { /* System Health */ } <
                    ...
                    div className = "space-y-5" > {
                        systemHealth.map((item, i) => {
                            ...
                            return ( <
                                ...
                                /div> <
                                span className = { `text-xs font-bold px-2 py-1 rounded-md ${item.status === 'success' ? 'bg-emerald-50 text-emerald-600' : item.status === 'warning' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}` } > { item.value } <
                                /span> <
                                /div>
                            );
                        })
                ...
                const TutorApprovalsView = () => ( <
                        ...
                        tutorsLoading ? ( <
                                ...
                                tbody className = "divide-y divide-slate-50" > {
                                    filteredTutors.length > 0 ? filteredTutors.map((tutor) => ( <
                                            ...
                                            div className = "flex items-center space-x-3" >
                                            <
                                            div className = "w-10 h-10 rounded-xl bg-brand-100 flex items-center justify-center text-brand-600 font-bold text-xs" > { getInitials(tutor.name) } <
                                            /div> <
                                            div >
                                            <
                    ...
                    // ── Render ──
                    return ( <
                        div className = "flex min-h-screen bg-slate-50 font-sans" > { /* Sidebar with inline rendering for click handling */ } <
                        aside className = { `fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}` } >
                        <
                        div className = "flex flex-col h-full" >
                        <
                        ...
                        nav className = "flex-1 px-4 py-6 space-y-1 overflow-y-auto" >
                        <
                        p className = "px-2 mb-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest" > Main Menu < /p> { renderNavLinks() } <
                        /nav>
                        ...
                        { /* Tutor Detail Modal */ } {
                            showDetailsModal && selectedTutor && ( <
                                TutorDetailModal tutor = { selectedTutor }
                                onClose = {
