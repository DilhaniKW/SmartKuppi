      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 transition-transform duration-300 lg:translate-x-0 lg:static ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-slate-800">
            <button onClick={handleLogout} className="flex items-center space-x-3 px-4 py-3 w-full text-slate-400 hover:text-rose-400 hover:bg-rose-400/10 rounded-xl transition-all"><LogOut className="h-5 w-5" /><span>Sign Out</span></button>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-30 flex-shrink-0">
          <div className="flex items-center space-x-4">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="lg:hidden p-2 text-slate-600">{isSidebarOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}</button>
            <div className="hidden md:flex items-center bg-slate-100 rounded-xl px-4 py-2 w-64 lg:w-96"><Search className="h-4 w-4 text-slate-400 mr-2" /><input type="text" placeholder="Search courses, resources..." className="bg-transparent border-none focus:ring-0 text-sm w-full" /></div>
          </div>
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="w-10 h-10 rounded-xl bg-brand-500 flex items-center justify-center text-white font-bold">{getInitials()}</div>
            </div>
          </div>
        </header>
          ...
          <div className="flex flex-col min-h-full">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    <div className="flex items-center justify-between">
                      ...
                      <Link to="/courses" className="text-brand-600 text-sm font-bold flex items-center">View All <ArrowUpRight className="h-4 w-4 ml-1" /></Link>
                    </div>
                    {enrolledCourses.length > 0 ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {enrolledCourses.map((course) => (
                          <Link key={course._id} to={`/student/courses/${course._id}`} className="block">
                            <motion.div whileHover={{ y: -5 }} className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden group border-b-4 border-b-brand-500">
                              <div className="relative h-40 overflow-hidden">
                                ...
                                <div className="absolute bottom-4 left-4">
                                  <span className="text-[10px] font-bold text-white uppercase tracking-widest bg-brand-500 px-2 py-1 rounded">{course.tutor?.name || 'Tutor'}</span>
                                </div>
                              </div>
                              <div className="p-6">
                                ...
                                <div className="mt-4 space-y-2">
                                  ...
                                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                                    <motion.div initial={{ width: 0 }} animate={{ width: `${course.progress || 0}%` }} className="h-full bg-brand-500 rounded-full"></motion.div>
                                  </div>
                                </div>
                                <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between">
                                  <div className="flex items-center text-[10px] text-slate-400 font-bold uppercase tracking-wider"><Clock className="h-3 w-3 mr-1 text-brand-500" /><span>Next: {course.nextLesson || 'No upcoming'}</span></div>
                                  <button className="p-2 bg-brand-50 text-brand-600 rounded-lg hover:bg-brand-100 transition-colors"><ChevronRight className="h-4 w-4" /></button>
                                </div>
                              </div>
                            </motion.div>
                          </Link>
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                      <div className="flex items-center justify-between mb-6"><h2 className="text-lg font-bold text-slate-900">Learning Streak 🔥</h2><span className="text-brand-600 font-bold text-sm">{stats.learningStreak} Days</span></div>
                      <div className="flex items-end justify-between h-24 gap-2">
                        {[40, 70, 45, 90, 65, 30, 80].map((height, i) => (
                          <div key={i} className="flex-1 flex flex-col items-center gap-2">
                            <motion.div initial={{ height: 0 }} animate={{ height: `${height}%` }} className={`w-full rounded-t-lg ${i === 6 ? 'bg-brand-500' : 'bg-slate-100'}`}></motion.div>
                          ...
                          {upcomingLessons.map((lesson) => (
                            <div key={lesson.id} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-brand-200 transition-colors group">
                              <div className="flex items-start justify-between">
                                <div><span className="text-[10px] font-bold uppercase tracking-wider text-brand-600">{lesson.subject}</span><h3 className="font-bold text-slate-900 mt-1 group-hover:text-brand-600">{lesson.title}</h3><p className="text-xs text-slate-500 mt-1">with {lesson.tutor}</p></div>
                                <div className="p-2 bg-white text-slate-400 rounded-xl shadow-sm"><Calendar className="h-4 w-4" /></div>
                              </div>
                              <div className="mt-4 flex items-center justify-between">
                                <div className="flex items-center text-xs font-bold text-slate-500"><Clock className="h-3 w-3 mr-1 text-brand-500" />{lesson.time}</div>
                                <button className="px-3 py-1.5 bg-brand-600 text-white text-[10px] font-bold rounded-lg hover:bg-brand-700">Join</button>
                              </div>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="space-y-4">
                          {recommendedResources.map((res) => (
                            <div key={res.id} className="flex items-center justify-between group">
                              <div className="flex items-center space-x-3">
                                <div className="p-2 bg-slate-50 text-slate-500 rounded-xl group-hover:bg-brand-50 group-hover:text-brand-600"><BookOpen className="h-4 w-4" /></div>
                              ...
                              <button className="p-2 text-slate-400 hover:text-brand-600 hover:bg-brand-50 rounded-lg"><Download className="h-4 w-4" /></button>
                            </div>
                          ))}
                        </div>
            ...
            <footer className="bg-white border-t border-slate-100 py-6 px-8 flex-shrink-0">
              <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center space-x-2"><GraduationCap className="h-5 w-5 text-brand-500" /><span className="font-bold text-slate-900">Smart<span className="text-brand-500">Kuppi</span></span><span className="text-xs text-slate-400 ml-2">© 2024 Student Portal v1.0.5</span></div>
                <div className="flex items-center space-x-6 text-xs font-bold text-slate-400 uppercase tracking-widest"><button className="hover:text-brand-500">Help Center</button><button className="hover:text-brand-500">Tutor Support</button><button className="hover:text-brand-500">Terms</button></div>
              </div>
            </footer>
          </div>
        </main>
