...
import { Calendar, Clock, Download, FileText, MessageSquare, Video } from 'lucide-react';
...
const StudentCourseDetail = () => {
    ...
    const fetchConversation = useCallback(async () => {
        ...
        try {
            const res = await fetch(
                `${API_BASE_URL}/messages?course=${course._id}&user=${course.tutor._id}`,
                { headers: { Authorization: `Bearer ${token}` } }
            );
    ...
    }, [course, activeTab, fetchConversation]);
                                    ...
                                    >
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500">
                                                {(resource.fileType || 'file').toUpperCase()} • {resource.downloads || 0} downloads
                                            </p>
                                        </div>
                                        <a
