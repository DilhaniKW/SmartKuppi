...
const TutorSchedule = ({ onBack }) => {
    ...
    const getUpcomingLessons = () => {
        const now = new Date();
        return lessons.filter(lesson => new Date(lesson.date) > now).slice(0, 5);
    };
    ...
    return ( <
        ...
        p className = "text-2xl font-bold text-emerald-600" > { lessons.filter(l => new Date(l.date) > new Date()).length } <
        /p> <
        /div> <
        div className = "bg-white p-5 rounded-3xl border border-slate-100 shadow-sm" >
        <
        p className = "text-xs font-bold text-slate-400 uppercase tracking-widest mb-1" > Total Students < /p> <
        p className = "text-2xl font-bold text-blue-600" > { lessons.reduce((sum, l) => sum + (l.enrolledCount || 0), 0) } <
        /p> <
        /div> <
        /div>
        ...
        onChange = {
            (e) => setFilterCourse(e.target.value) }
...

        {
            loading ? ( <
                ...
                > {
                    view === 'calendar' ? ( <
                        div className = "grid grid-cols-1 lg:grid-cols-3 gap-8" > { /* Calendar */ } <
                ...
                ` } < /style> <
                        ...
                        <
                        h3 className = "font-bold text-slate-900" > { format(selectedDate, 'EEEE, MMMM d, yyyy') } <
                        /h3> <
                        /div> <
                        /div> <
                        div className = "p-6 space-y-4 max-h-[500px] overflow-y-auto" > {
                            filteredLessons.length > 0 ? (
                                filteredLessons.map(lesson => ( <
                                    ...
                                    <
                                    /div> { getStatusBadge(lesson.date) } <
                                    /div> <
                                    ...
                                    /div> {
                                        lesson.meetingLink && ( <
                                            ...
                                            <
                                            /a> <
                                            Link to = { `/tutor/courses/${lesson.courseId}` }
                                ...
                                <
                                p className = "text-slate-500" > No lessons scheduled
                                for this day < /p> <
                                Link to = "/tutor/create-lesson"
                        ...
                        div className = "divide-y divide-slate-100" > {
                            lessons.filter(l => new Date(l.date) >= new Date()).length === 0 ? ( <
                                ...
                                lessons.filter(l => new Date(l.date) >= new Date()).sort((a, b) => new Date(a.date) - new Date(b.date)).map(lesson => ( <
                                    ...
                                    <
                                    CalendarIcon className = "h-3 w-3" / > { new Date(lesson.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }) } <
                                    /span> <
                                    span className = "flex items-center gap-1" >
                                    <
                                    Clock className = "h-3 w-3" / > { formatTime(lesson.date) } <
                                    /span> <
                                    span className = "flex items-center gap-1" >
                                    <
                                    ...
                                    span className = "flex items-center gap-1" >
                                    <
                                    Users className = "h-3 w-3" / > { lesson.enrolledCount || 0 }
                                    enrolled <
                                    /span> <
                                    /div> <
                                    /div> <
